...
| 可观测 | LLM 调用日志、工具链路、延迟、错误、request id、结构化审计事件，审计事件哈希链校验，以及可签名审计锚点证据 |
...
| 质量 | pytest 覆盖主链路，ruff lint/format，前端编译检查，Docker Compose 配置校验；评测脚本输出 JSON/Markdown 报告，并按风险准确率、违规类型准确率、引用覆盖率和高风险召回率阈值失败退出；smoke test 从 HTTP 外部验证已启动后端主链路、审计链校验和审计锚点校验；GitHub Actions 自动运行测试、评测门禁、运行时 smoke test 和前后端 Docker 镜像构建 |
...
| 审计可信度 | 新审计事件带 previous_hash/event_hash，可校验内容与链路是否被篡改；管理员可创建审计锚点 JSON，记录事件前缀 SHA256、最后事件 hash、链校验摘要和 schema 版本，并可用 `BACKUP_SIGNING_KEY` 做 HMAC-SHA256 签名；锚点校验会复核 manifest、签名和当前数据库审计前缀 | 生产化把锚点周期性写入 WORM 存储、SIEM、对象存储版本锁或云审计服务，并建立不可抵赖的外部时间戳 |
...
> 这个项目当前定位是企业客服质检 POC，而不是直接替代生产质检系统。它已经具备 RAG、结构化质检报告、引用校验、中英文界面、工单与知识库文档脱敏、知识库提示注入风险扫描与隔离复核、历史知识库隐私扫描/补救、日志、Prometheus 指标与告警规则、Prometheus/Alertmanager 示例部署、Nginx 统一入口和 HTTPS 示例配置、user/admin API key、API key SHA-256 哈希配置、可信网关身份头认证、接口限流、生产配置自检、安全状态总览、前端运维中心、数据库 schema 版本台账、SQLite WAL/busy timeout/quick check 运行自检、审计事件哈希链、可签名审计锚点证据、后台批量任务查询/取消/超时/幂等提交/活跃容量限制/中断恢复标记、报告/任务结果隔离、人工复核 API、数据保留清理、本地备份快照、文件级 SHA256 校验、manifest HMAC 签名、只读恢复演练、健康检查、运行时 smoke test、样例工单评测门禁、Docker Compose 部署、非 root 容器和 GitHub Actions 质量门禁；如果要生产化，我会优先做内置 OIDC/权限模型、PostgreSQL/Alembic、Redis/Celery 队列、对象存储、正式证书管理、WORM/SIEM 审计归档和企业告警路由。
